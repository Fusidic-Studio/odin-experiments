...
main :: proc() {
	...
	log.info("Initialising GLFW and WebGPU...")
	...
	on_device :: proc "c" (
		...
		uniformBuffer := wgpu.DeviceCreateBuffer

		wgpu.DeviceCreateBindGroup(
			state.device,
			{
				layout = wgpu.GetBinGroupLayout(0),
				entries = wgpu.BinG[{binding = 0, resource = uniformBuffer}],
			},
		)
