...
CreateLight :: proc(
	...
	if lightsCount < MAX_LIGHTS {
		...
		light.enabledLoc = i32(
			rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].enabled", lightsCount)),
		)
		light.typeLoc = i32(
			rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].type", lightsCount)),
		)
		light.positionLoc = i32(
			rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].position", lightsCount)),
		)
		light.targetLoc = i32(
			rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].target", lightsCount)),
		)
		light.colorLoc = i32(
			rl.GetShaderLocation(shader, rl.TextFormat("lights[%i].color", lightsCount)),
		)
